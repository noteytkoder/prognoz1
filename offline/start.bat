...
if not exist requirements.txt (
    echo [INFO] requirements.txt not found, creating one with required dependencies...
    (
        echo pandas
        echo requests
        echo plotly
...
start "" python offline_dash.py
if errorlevel 1 (
    echo [ERROR] Failed to launch application. Check logs/offline.log for details.
    pause
    exit /b 1
)
...
echo.
echo Application launched. Check http://localhost:8051 in your browser.
echo Logs saved to logs/offline.log. If errors occur, send logs/offline.log and requirements_freeze.txt.
