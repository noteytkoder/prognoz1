...
def train_model(df, n_estimators=None, max_depth=None):
    ...
    try:
        ...
        # Преобразование max_depth: если 0, то устанавливаем None
        max_depth = max_depth or config["model"]["max_depth"]
        if max_depth == 0:
            logger.info("max_depth is 0, setting to None for unlimited tree depth")
            max_depth = None
...
def train_hourly_model(df):
    ...
    try:
        ...
        # Преобразование hourly_max_depth: если 0, то устанавливаем None
        max_depth = config["model"]["hourly_max_depth"]
        if max_depth == 0:
            logger.info("hourly_max_depth is 0, setting to None for unlimited tree depth")
            max_depth = None
        elif not isinstance(max_depth, (int, type(None))) or (isinstance(max_depth, int) and max_depth < 1):
